...
match = re.search(r'id:\s*"2".*?content:\s*`(.*?)`,', blog_data, re.DOTALL)
...
diff = difflib.unified_diff(db_lines, file_lines, fromfile='DB_content', tofile='v3_content', lineterm='')
diff_output = list(diff)

print(f"Total diff lines: {len(diff_output)}")
for line in diff_output[:100]: # Print first 100 lines of diff
    print(line)
